package com.xirc.milicraft.client.animation;
...
public final class MilicraftAnimations {
    private static final ResourceLocation CONTROLLER_ID = Milicraft.id("animation_controller");
    ...
    private static AdjustmentModifier gunHoldModifier(AbstractClientPlayer player) {
        return new GunArmModifier(partName -> {
            ...
            float sideAim = SIDE_AIM.computeIfAbsent(player, MilicraftAnimations::targetSideAim);
    ...
    private static Map<String, KeyframeAnimation> loadLocalAnimations(String file) {
        ...
        Optional<net.minecraft.server.packs.resources.Resource> resource = Minecraft.getInstance()
                .getResourceManager()
                .getResource(Milicraft.id("player_animations/" + file + ".json"));
        if (resource.isEmpty()) {
            return animations;
        }
