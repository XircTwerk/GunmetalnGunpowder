...
public class BerettaItem extends AbstractGunItem {
    ...
    protected SoundEvent fireSound() {
        return MilicraftSoundRegistry.BERETTA_M9_SHOT.get();
    }
    ...
    protected SoundEvent reloadPartSound(String part) {
        return switch (part) {
            case "reload_start", "deload_start" -> MilicraftSoundRegistry.WEAP_BOLT_OUT.get();
            case "reload_load", "deload_load" -> MilicraftSoundRegistry.WEAP_MAGIN_PLASTIC.get();
            case "reload_end", "deload_end" -> MilicraftSoundRegistry.WEAP_BOLT_OUT.get();
            default -> null;
        };
    }
    ...
    protected SoundEvent reloadPartEndSound(String part) {
        return "reload_start".equals(part) || "deload_start".equals(part)
                ? MilicraftSoundRegistry.WEAP_MAGDROP_PLASTIC.get()
                : null;
    }
